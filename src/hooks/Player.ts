...
import { getFmTrash, getPersonalFM } from '@/api/others';
...
function setTitle(track: { id?: number; name?: any; ar?: any; } | null) {
...
function setTrayLikeState(isLiked: any) {
  if (isCreateTray) {
...
export class Player {
  ...
  constructor() {
    ...
    // init
    this._init();
    Object.defineProperty(window, 'yesplaymusic', {
      value: { player: this },
      writable: true,
      configurable: true
    })
  ...
  _getAudioSourceFromNetease(track: any) {
    if (useIsAccountLoggedIn.value) {
      return getMP3(track.id).then(({ data }) => {
        ...
        if (settings.value.automaticallyCacheSongs) {
          cacheTrackSource(track, source, data.data[0].br);
        }
        return source;
      });
    } else {
  ...
  async playNextFMTrack() {
    ...
    if (!this._personalFMNextTrack) {
      ...
      for (; retryCount >= 0; retryCount--) {
        result = (await getPersonalFM().catch(() => null))?.data;
        if (!result) {
  ...
  playAlbumByID(id: number, trackID = 'first') {
    getAlbum(id).then(({ data }) => {
      let trackIDs = data.songs.map((t: any) => t.id);
      this.replacePlaylist(trackIDs, id, 'album', trackID);
    });
  }
  playPlaylistByID(id: number, trackID = 'first', noCache = false) {
    console.debug(
  ...
  playArtistByID(id: number, trackID = 'first') {
  ...
  playIntelligenceListById(id: any, trackID = 'first', noCache = false) {
    getPlaylistDetail(id, noCache).then(({ data }) => {
