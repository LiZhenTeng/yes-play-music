export function lyricParser(lrc: { lrc: { lyric: any; }; tlyric: { lyric: any; }; lyricUser: any; transUser: any; }) {
    return {
...
function parseLyric(lrc: string) {
     ...
     */
    const binarySearch = (lyric: { rawTime?: any; time: any; content?: any; }) => {
        let time = lyric.time;
    ...
    for (const line of lrc.trim().matchAll(extractLrcRegex)) {
        const { lyricTimestamps, content } = line.groups as { [key: string]: string };

        for (const timestamp of lyricTimestamps.matchAll(extractTimestampRegex)) {
            const { min, sec, ms } = timestamp.groups as { [key: string]: string; };
