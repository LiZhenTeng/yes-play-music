<template>
    <div v-show="show" class="playlist">
        <div v-if="specialPlaylistInfo === undefined && !isLikeSongsPage" class="playlist-info">
            <Cover :id="playlist.id" :image-url="useResizeImage(playlist.coverImgUrl, 1024)" :show-play-button="true"
                :always-show-shadow="true" :click-cover-to-play="true" :fixed-size="288" type="playlist"
                :cover-hover="false" :play-button-size="18" @click.right.native="openMenu" />
            <div class="info">
                <div class="title" @click.right="openMenu"><span v-if="playlist.privacy === 10" class="lock-icon">
                ...
                <div class="artist">
                    ...
                    <a v-else :href="`https://music.163.com/#/user/home?id=${playlist.creator.userId}`"
                        target="blank">{{ playlist.creator.nickname }}</a>
                </div>
                <div class="date-and-count">
                    {{ $t('playlist.updatedAt') }}
                    {{ useFormatDate(playlist.updateTime) }} · {{ playlist.trackCount }}
                    {{ $t('common.songs') }}
                </div>
                ...
                <div class="buttons">
                    ...
                    <ButtonTwoTone v-if="playlist.creator.userId !== data.user.userId"
                        :icon-class="playlist.subscribed ? 'heart-solid' : 'heart'" :icon-button="true"
                        :horizontal-padding="0" :color="playlist.subscribed ? 'blue' : 'grey'"
                        :text-color="playlist.subscribed ? '#335eea' : ''" :background-color="
            ...
            <div v-if="displaySearchInPlaylist" class="search-box">
                <div class="container" :class="{ active: inputFocus }">
                    ...
                    <div class="input">
                        <input v-model.trim="inputSearchKeyWords" v-focus
                            :placeholder="inputFocus ? '' : $t('playlist.search')" @input="inputDebounce()"
        ...
        <div v-if="isLikeSongsPage" class="user-info">
            <h1>
                <img class="avatar" :src="useResizeImage(data.user.avatarUrl)" loading="lazy" />
        ...
        <ContextMenu ref="playlistMenu">
            ...
            <div v-if="playlist.creator.userId === data.user.userId" class="item" @click="deletePlaylistFn">删除歌单</div>
        </ContextMenu>
    </div>
...
import { getPlaylistDetail, subscribePlaylist, deletePlaylist, } from '@/api/playlist';
...
import { useResizeImage, useFormatDate } from '@/utils/common'

const specialPlaylist: { [k: string]: any } = {
    2829816518: {
        name: '欧美私人订制',
...
const { player, showToast } = indexStore;
...
const loadData = (id: number, next: Function | undefined = undefined) => {
    ID.value = id;
...
onMounted(() => {
    if (route.name === 'likedSongs') {
        loadData(data.value.likedSongPlaylistID as number);
    } else {
